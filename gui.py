...
def main():    
    ...
    def update():
        f=open("bbox_values.txt", "r")    
        gamearea =f.read()
        #filemenu.delete(1)
        filemenu.add_command(label=gamearea)
        filemenu.delete(3)
    ...
    filemenu.add_command(label="Set",command=snip)
    filemenu.add_separator()
    filemenu.add_command(label="See coordinates",command=update)
    ##filemenu.add_command(label="Not SET", command=update)
    menubar.add_cascade(label="Set Game Area", menu=filemenu , command=update)
    ...
    L_Question = Label(top, text="Question",relief="solid",padx=80).grid(row=0,columnspan=4,pady=(5,0))
    L_Guess=Label(top,text="Guess",padx=10).grid(row=0,column=4,pady=(5,0))

    L_Control = Label(top, text="Control",).grid(row=0,column=5,pady=(5,0))
    ...
    B_Reset=Button(top, text ="Reset",padx=4, command=restart_program).grid(row=2,column=5)
